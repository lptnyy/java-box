...
public class ConnectPoolWatch implements Watcher {
    ...
    String path;
    ZooKeeper zooKeeper;
    public ConnectPoolWatch(String path, ZooKeeper zooKeeper){
        this.path = path;
    ...
    public static void initWater(String str, ZooKeeper zooKeeper){
        if(!watcherData.containsKey(str)){
            ...
            try {
                zooKeeper.getData(ZkConfig.APP_CONNECT_POOL+"/"+str, new WatcherData(), null);
            } catch (KeeperException e) {
                e.printStackTrace();
            } catch (InterruptedException e) {
                e.printStackTrace();
